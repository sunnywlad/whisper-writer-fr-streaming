#!/usr/bin/env python3
#
# Vendored from https://github.com/ufal/whisper_streaming
# Upstream file: whisper_online.py
# Upstream commit: 9c3860c56baef7098aaeb3ee48dbc28d9741546f (2025-01-07)
# License: MIT, (c) 2023 UFAL. Full text in src/vendor/LICENSE.whisper_streaming.
#
# WhisperWriter modification (the ONLY change to upstream source):
#   `import librosa` was moved from module scope into `load_audio()`.
#   librosa is used by that one file-loading helper, which WhisperWriter never
#   calls: its audio arrives from sounddevice already at 16 kHz mono float32.
#   Keeping the import at module scope would make this module unimportable
#   without a heavy dependency we have no use for.
#
# Everything else below is upstream, byte for byte. Adaptations WhisperWriter
# needs (injecting an already-loaded WhisperModel, CPU decode options, the
# temperature fallback) live in src/streaming_asr.py as a subclass, so this
# file stays trivially diffable against upstream.
import sys
import numpy as np
...
def load_audio(fname):
    import librosa  # WhisperWriter: lazy, see the note at the top of the file.
